...
- 家长浏览完成后优先使用系统录屏通知停止屏幕采集，再返回小伴查看状态、群校验、时间边界并自动整理；应用内结束按钮仅作为兜底。
...
- 真机仍需验证不同 HarmonyOS / 微信版本的 AVScreenCapture、系统录屏通知停止、OCR 与后台切换/返回应用行为；
- 需要维护 SourceProfile；
...
拒绝。FLOAT_VIEW / SYSTEM_FLOAT_WINDOW 属于受限或高敏感悬浮能力，会扩大应用权限面并增加应用市场审核风险。当前场景并不要求在微信界面上覆盖操作控件。优先复用系统录屏通知的停止入口，返回小伴后完成剩余证据处理即可满足业务闭环。
