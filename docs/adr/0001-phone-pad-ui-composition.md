...
4. **导航形态与业务内容布局分开判断。**
   - Window size class 可以继续决定底部导航 / 侧边导航等 Shell 级行为；
   - 业务页面不得把 `COMPACT / MEDIUM / EXPANDED` 直接等同于单栏 / 双栏。
...
- 旧页面中直接判断 `WindowSizeClass` 的布局逻辑需要逐步移除。
...
- 统一 `LayoutPolicy` 判断多栏能力。

当前不强制依赖 API 26+ 的 `ContainerReader`。未来最低 API 升级后，可以替换容器测量实现，但页面仍只依赖 `LayoutCapability`，不需要重写业务页面。

## Sources

- V2 Product Baseline: `docs/product/product-feature-list-v2.md`
- V2 Frontend Design: `docs/architecture/frontend-technical-design-v2.md`
- V2 System Design: `docs/architecture/system-technical-design-v2.md`
