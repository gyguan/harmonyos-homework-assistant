...
| `HomeworkStore` settings/student context | MIGRATE（边界完成） | `FamilyContextRepository` + `FamilyContextLocalDataSource` | Repository/Remote/Shell 不再直接访问 Store；待 local adapter 删除 | Final Cleanup |
