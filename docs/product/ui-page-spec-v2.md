...
- 点击任务主按钮直接执行必要的 START 命令并进入 Study Workspace；
- IN_PROGRESS / READY_TO_SUBMIT 不重复执行 START，直接进入当前学习或提交状态；
- 不在首页直接展开完整 AI 对话；
- 已提交 / 已完成任务不出现在首页，统一到作业列表历史区查看。

### Pad 增强

Pad 可在空间充足时使用“先做这项 + 接下来”双区，但第一项任务不被压缩。
...
- 活跃状态统一进入“要做的”，不拆成“需要处理 / 待开始”等多个重复区块；
- “已提交 / 已完成”合并为一条历史入口，默认收起；
- 底层筛选与 Assignment 状态保持不变。
...
默认保持居中可读的单任务内容。学生主动点击“问小伴”且空间足够时：
...
- 首页只保留“布置作业 / 语音作业 / 作业收件箱”三个高频入口；
- “需要我处理”、今日统计、进度统计统一移入 Progress；
- 不展示 KPI 卡、最近动态、待办列表或重复的进度信息；
- 主入口强调操作意图，说明文案保持简短；
- Phone 两个创建入口并排，收件箱作为次级全宽入口。

### Pad 增强

空间足够时三个入口可横向排布，但保持同一信息层级，不增加额外业务模块。
...
- 原家长首页的今日统计迁移到 Progress；
- 默认日期为今天，因此首次进入时统计口径与原首页“今日统计”一致；
- 统计数据与列表共享同一日期、科目范围；
- 点击“全部任务 / 待完成 / 待验收 / 已完成”后，下面列表立即按对应状态联动；
- 修改日期或科目后，统计数字与列表同时切换到新的查询范围；
- Progress 是家长查看状态、处理任务、批量管理的统一页面。
